(function () {
	...
	function catchMouseEvents() {
		...
		if (_.mouseDown) {
			if (shootCounter > 7) {
				var jsonString = JSON.stringify(new dataObject("bul", {
					xPos: pl.xPos,
					yPos: pl.yPos,
					angle: _.giveRadsBetweenPoints(canvas.clientWidth / 2, canvas.clientHeight / 2, _.mouseCurrent.x - canvas.offsetLeft, _.mouseCurrent.y - canvas.offsetTop)
				}));
